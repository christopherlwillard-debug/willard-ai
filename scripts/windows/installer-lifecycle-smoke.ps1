...
$Createdb = Join-Path $env:ProgramFiles "PostgreSQL\16\bin\createdb.exe"
$Dropdb = Join-Path $env:ProgramFiles "PostgreSQL\16\bin\dropdb.exe"
$TempRoot = Join-Path $env:RUNNER_TEMP "willard-installer-lifecycle-$([guid]::NewGuid().ToString())"
$UserName = "willard_lifecycle_$([guid]::NewGuid().ToString("N").Substring(0, 8))"
